...
def entropy_map(a, dim, normalize=True, temperature=1.0):
    """
    计算熵图（支持温度缩放和可选归一化）
    a: [B, C, H, W] softmax 概率
    dim: 类别维度，一般是1
    normalize: 是否归一化到 [0, 1] 区间
    temperature: 温度系数，>1 会让分布更平滑，<1 更尖锐
    """
    # 温度缩放，减少过于尖锐的概率分布带来的不稳定
    if temperature != 1.0:
        a = torch.softmax(torch.log(a + 1e-10) / temperature, dim=dim)

    em = -torch.sum(a * torch.log2(a + 1e-10), dim=dim)

    # 归一化到 [0, 1]，保证不同数据集的熵值分布更可比
    if normalize:
        em = em / np.log2(a.shape[dim])
...
def adjust_thresholds(cls_thresholds, average_confidence,adjustment_factor,set_threshold):
    ...
    for i in range(len(cls_thresholds)):
        ...
        if avg_conf > high_threshold:
            ...
            new_thresholds[i] = set_threshold  # 防止阈值过低
        # elif avg_conf < low_threshold:
        #     new_thresholds[i] = avg_conf
        #     其他置信度（包括低置信度和中置信度均设为固定阈
